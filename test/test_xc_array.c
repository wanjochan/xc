...
static void test_array_create() {
    ...
    xc_object_t *empty_array = xc_array_create(rt);
    TEST_ASSERT(xc_is_array(rt, empty_array), "Should be an array type");
    TEST_ASSERT(xc_array_length(rt, empty_array) == 0, "Empty array should have length 0");
    
    // Create an array with initial values
    xc_object_t *values[3];
    values[0] = xc_number_create(rt, 10);
    values[1] = xc_string_create(rt, "test");
    values[2] = xc_boolean_create(rt, 1);
    
    xc_object_t *array = xc_array_create_with_values(rt, values, 3);
    TEST_ASSERT(xc_is_array(rt, array), "Should be an array type");
    TEST_ASSERT(xc_array_length(rt, array) == 3, "Array should have length 3");
    
    // Check element types
    TEST_ASSERT(xc_is_number(rt, xc_array_get(rt, array, 0)), "First element should be a number");
    TEST_ASSERT(xc_is_string(rt, xc_array_get(rt, array, 1)), "Second element should be a string");
    TEST_ASSERT(xc_is_boolean(rt, xc_array_get(rt, array, 2)), "Third element should be a boolean");
    
    // Check element values
    TEST_ASSERT(xc_number_value(rt, xc_array_get(rt, array, 0)) == 10, "First element should be 10");
    TEST_ASSERT(strcmp(xc_string_value(rt, xc_array_get(rt, array, 1)), "test") == 0, "Second element should be 'test'");
    TEST_ASSERT(xc_boolean_value(rt, xc_array_get(rt, array, 2)) == 1, "Third element should be true");
    
    // Test out of bounds access
    TEST_ASSERT(xc_is_null(rt, xc_array_get(rt, array, 3)), "Out of bounds access should return null");
    TEST_ASSERT(xc_is_null(rt, xc_array_get(rt, array, -1)), "Negative index access should return null");
