...
static void test_null_simple(void) {
    ...
    xc_val null_val = (xc_val)xc_null_create(&xc);
    if (null_val == NULL) {
        ...
        /* 3. 类型检查测试 */
        int type = xc.type_of(null_val);
        TEST_ASSERT(type == XC_TYPE_NULL, "Type of null value is XC_TYPE_NULL");
        TEST_ASSERT(xc.is(null_val, XC_TYPE_NULL), "is() correctly identifies NULL type");
        TEST_ASSERT(!xc.is(null_val, XC_TYPE_NUMBER), "NULL is not a NUMBER");
    }
...
static void test_boolean_simple(void) {
    ...
    xc_val true_val = (xc_val)xc_boolean_create(&xc, XC_TRUE);
    xc_val false_val = (xc_val)xc_boolean_create(&xc, XC_FALSE);
    ...
    if (true_val == NULL || false_val == NULL) {
        ...
        /* 3. 类型检查测试 */
        TEST_ASSERT(xc.type_of(true_val) == XC_TYPE_BOOL, "Type of TRUE is XC_TYPE_BOOL");
        TEST_ASSERT(xc.type_of(false_val) == XC_TYPE_BOOL, "Type of FALSE is XC_TYPE_BOOL");
        
        TEST_ASSERT(xc.is(true_val, XC_TYPE_BOOL), "is() correctly identifies TRUE as BOOL");
        TEST_ASSERT(xc.is(false_val, XC_TYPE_BOOL), "is() correctly identifies FALSE as BOOL");
        
        TEST_ASSERT(!xc.is(true_val, XC_TYPE_NUMBER), "TRUE is not a NUMBER");
    }
...
static void test_boolean_operations(void) {
    ...
    if (not_result != NULL) {
        TEST_ASSERT(xc.is(not_result, XC_TYPE_BOOL), "NOT result is a boolean");
        /* 更多的逻辑测试可以在这里添加 */
    } else {
    ...
    if (and_result != NULL) {
        TEST_ASSERT(xc.is(and_result, XC_TYPE_BOOL), "AND result is a boolean");
        /* 更多的逻辑测试可以在这里添加 */
    } else {
...
static void test_number_simple(void) {
    ...
    xc_val int_val = (xc_val)xc_number_create(&xc, 42);
    xc_val float_val = (xc_val)xc_number_create(&xc, 3.14159);
    xc_val neg_val = (xc_val)xc_number_create(&xc, -10);
    ...
    if (int_val == NULL || float_val == NULL || neg_val == NULL) {
        ...
        /* 3. 类型检查测试 */
        TEST_ASSERT(xc.type_of(int_val) == XC_TYPE_NUMBER, "Type of integer is XC_TYPE_NUMBER");
        TEST_ASSERT(xc.type_of(float_val) == XC_TYPE_NUMBER, "Type of float is XC_TYPE_NUMBER");
        TEST_ASSERT(xc.type_of(neg_val) == XC_TYPE_NUMBER, "Type of negative is XC_TYPE_NUMBER");
        
        TEST_ASSERT(xc.is(int_val, XC_TYPE_NUMBER), "is() correctly identifies integer as NUMBER");
        TEST_ASSERT(!xc.is(int_val, XC_TYPE_BOOL), "Number is not a BOOL");
    }
...
static void test_number_operations(void) {
    ...
    xc_val num1 = (xc_val)xc_number_create(&xc, 10);
    xc_val num2 = (xc_val)xc_number_create(&xc, 5);
    ...
    /* 尝试加法操作 */
    xc_val add_result = xc.call(num1, "add", num2);
    if (add_result != NULL) {
        TEST_ASSERT(xc.is(add_result, XC_TYPE_NUMBER), "Addition result is a number");
        /* 更多的算术测试可以在这里添加 */
    } else {
    ...
    /* 尝试减法操作 */
    xc_val sub_result = xc.call(num1, "subtract", num2);
    if (sub_result != NULL) {
        TEST_ASSERT(xc.is(sub_result, XC_TYPE_NUMBER), "Subtraction result is a number");
        /* 更多的算术测试可以在这里添加 */
    } else {
...
static void test_string_simple(void) {
    ...
    xc_val empty_str = (xc_val)xc_string_create(&xc, "");
    xc_val hello_str = (xc_val)xc_string_create(&xc, "Hello, World!");
    ...
    if (empty_str == NULL || hello_str == NULL) {
        ...
        /* 3. 类型检查测试 */
        TEST_ASSERT(xc.type_of(empty_str) == XC_TYPE_STRING, "Type of empty string is XC_TYPE_STRING");
        TEST_ASSERT(xc.type_of(hello_str) == XC_TYPE_STRING, "Type of non-empty string is XC_TYPE_STRING");
        
        TEST_ASSERT(xc.is(empty_str, XC_TYPE_STRING), "is() correctly identifies empty string as STRING");
        TEST_ASSERT(xc.is(hello_str, XC_TYPE_STRING), "is() correctly identifies non-empty string as STRING");
        
        TEST_ASSERT(!xc.is(hello_str, XC_TYPE_NUMBER), "String is not a NUMBER");
    }
...
static void test_string_operations(void) {
    ...
    xc_val str1 = (xc_val)xc_string_create(&xc, "Hello");
    xc_val str2 = (xc_val)xc_string_create(&xc, "World");
    ...
    /* 尝试连接操作 */
    xc_val concat_result = xc.call(str1, "concat", str2);
    if (concat_result != NULL) {
        TEST_ASSERT(xc.is(concat_result, XC_TYPE_STRING), "Concatenation result is a string");
        /* 更多的字符串操作测试可以在这里添加 */
    } else {
    ...
    /* 尝试获取长度 */
    xc_val length_result = xc.call(str1, "length");
    if (length_result != NULL) {
        TEST_ASSERT(xc.is(length_result, XC_TYPE_NUMBER), "Length result is a number");
        /* 更多的字符串操作测试可以在这里添加 */
    } else {
