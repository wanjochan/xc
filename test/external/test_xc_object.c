...
void test_object_basic(void) {
    ...
    result = xc.dot(obj, "answer");
    TEST_ASSERT(result != NULL, "Number property retrieval failed");
    TEST_ASSERT(xc.is(result, XC_TYPE_NUMBER), "Number property type check failed");
...
void test_object_prototype(void) {
    ...
    xc_val result = xc.dot(obj, "objProperty");
